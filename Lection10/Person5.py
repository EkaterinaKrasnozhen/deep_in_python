...
class Hero(Person, Address, Weapon): # порядок важен для порядка ин циализации и поиска методов и атрибутов
    def __init__(self, power, name=None, race=None, speed=None, 
                country=None, city=None, street=None, left_hand=None, right_hand=None):
        self.power = power # функцию super не можем использовать она будет обращаться только к 1 родителю
        Person.__init__(self, name, race, speed)
        Address.__init__(self, country, city, street) # не очень удобная структра, если например изменится в Address набор параметров то тут будет ошибка и надо вручную добавлять
...
def add_many_ups(self):
    self.up += 1
    self.up = min(self.up, self._Person5__max_up * 2)

p1 = Hero('archery', 'Сильвана', 'Эльф', 120,
country='Эльфляндия', street='Ночного эльфа',
left_hand='Стрела')
print(f'{p1.say_address()}')#Адрес героя: Эльфляндия, ,Ночного эльфа это метод из наследования, пошел в класс Address и там нашел по цепочке слева направо
