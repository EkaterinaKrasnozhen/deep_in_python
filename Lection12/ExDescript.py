class Range:
    def __init__(self, min_value: int = None, max_value: int = None): # задаем границы или ничего
    ...
    def __delete__(self, instance):
        raise AttributeError(f'Свойство "{self.param_name}" нельзя удалять') # при попытке удаления ошибка

    def validate(self, value): # валидация
        if not isinstance(value, int):
            raise TypeError(f'Значение {value} должно быть целым числом')
        if self.min_value is not None and value < self.min_value: # проходит ли по нижней границе
            raise ValueError(f'Значение {value} должно быть больше 18 или равно {self.min_value}')# вызываем ошибку
        if self.max_value is not None and value >= self.max_value:# проходит ли по верхней границе
